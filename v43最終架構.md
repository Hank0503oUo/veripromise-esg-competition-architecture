...
這次最有趣的現象是：public leaderboard 低估了最終方法，但本地 validation / gate 分數反而更接近 private leaderboard。

| Evidence | Score / rank | Interpretation |
|---|---:|---|
| v5.3 local baseline | `0.6460` | 早期本地 validation 已估到 final private 區間 |
| v18 prior stack | `val WS 0.6412508` | 穩定的 grouped validation lower bound |
| v18C A+B+C ensemble | `val WS 0.6506880` | Engine C 後的主要本地估計帶 |
| v25 public known score | `public WS 0.6060570` | public leaderboard 明顯低於 local/private estimate |
| v43 final | `private 0.6457201`, rank `6/143`, public rank `59` | private 結果落在本地估計帶中間，且 public -> private 上升 53 名 |

這表示最後的決策不是單純賭榜，而是本地 grouped validation、單欄 ablation 與 deployment gate 對 private distribution 有較好的預測力。也解釋了為什麼許多 public 看似可嘗試的 EQ/LLM 分支最後沒有被合併。
